...
func recipeSQLTool(cfg Config, timeout time.Duration) Tool {
	return nativeTool("recipe_sql", GroupGTNHData, "Run one read-only SELECT or WITH SELECT against the indexed GTNH production database. Resolve items and fluids with resource_search, then query recipe_routes by exact output_resource_key and recipe_ingredients by selected recipe_id. Use recipe_compare for two-target feasibility comparisons and me_crafting for already installed ME patterns. Never broad-scan lower(output_name) or retry a timed-out broad query. recipe_routes excludes invalid, hidden, fake, disabled, and unresolved-input recipes. Duplicate result column names are preserved with _2, _3, and later suffixes.", timeout, object(
