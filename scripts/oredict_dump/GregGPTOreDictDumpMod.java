...
@Mod(
    modid = "greggpt_oredict_dump",
    name = "GregGPT OreDict Dump",
    version = "1.0.0",
    acceptableRemoteVersions = "*"
)
public final class GregGPTOreDictDumpMod {
  private static final AtomicBoolean WORKER_STARTED = new AtomicBoolean(false);
  private static final String DUMP_FILE_NAME = "greggpt_oredict_dump.tsv";
  private static final String LOG_FILE_NAME = "greggpt_oredict_dump.log";

  public GregGPTOreDictDumpMod() {
  ...
  private static void startDumpWorker(final String trigger) {
    if (!WORKER_STARTED.compareAndSet(false, true)) {
      appendLog("[GREGGPT] dump worker already started; ignoring trigger " + trigger);
      return;
    }
  ...
  private static void runDumpWorker(String trigger) {
    ...
    appendLog("[GREGGPT] dump worker started from " + trigger);
    ...
    for (int attempt = 1; attempt <= 300; attempt++) {
      ...
      try {
        ...
        appendLog("[GREGGPT] ore dict probe failed on attempt " + attempt + ": " + t);
      }
      ...
      if (loader != null && loader.hasReachedState(LoaderState.AVAILABLE) && oreNameCount > 0) {
        try {
          writeDump(outFile);
          appendLog("[GREGGPT] wrote ore dict dump to " + outFile.getAbsolutePath());
          System.out.println("[GREGGPT] wrote ore dict dump to " + outFile.getAbsolutePath());
          sleepQuietly(1500L);
          System.exit(0);
          return;
        } catch (Throwable t) {
          appendThrowable("[GREGGPT] failed to write ore dict dump", t);
