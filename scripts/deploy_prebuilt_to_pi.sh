...
rsync -av -e "$SSH_CMD" "$MC_RELAY_IMAGE_TAR" "$DISCORD_COMMANDS_IMAGE_TAR" "$PI_HOST:$REMOTE_IMAGE_DIR/"
...
cd '$PI_DIR/deploy'
podman-compose --env-file env/greggpt.env -f compose.yaml up -d --force-recreate --no-build --no-deps mc-relay discord-commands
